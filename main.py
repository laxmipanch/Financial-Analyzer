...
def run_analysis(company_name: str = "Apple") -> tuple[str, str]:
    global COMPANY_NAME
    COMPANY_NAME = company_name

    try:
        asyncio.run(main())  # This will generate the report
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file_prefix = company_name.lower().replace(" ", "_")
        # Find the latest file generated
        report_files = sorted(
            [f for f in os.listdir(OUTPUT_DIR) if f.startswith(output_file_prefix)],
            reverse=True
        )
        if report_files:
            ...
            return "", "Report generation failed: No report file found."
    except Exception as e:
